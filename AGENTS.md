...
- `worker/index.ts` answers `Accept: text/markdown` on page paths with the twin and hands every other request to static assets untouched; `worker/accept.ts` parses Accept per RFC 9110. `wrangler.jsonc` routes only `/`, `/work/*`, `/projects/*`, `/speaking/*` through it, and `public/_headers` sets `Vary: Accept` on both variants. The Worker has its own `worker/tsconfig.json` against types from `wrangler types` (generated by `bun run check`, not committed) and is covered by `bun test`
